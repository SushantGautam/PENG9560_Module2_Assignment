...
class RMED():
    ...
    def update_b_hat_star(self, i):
        ...
        p = dict([(x, (self.get_mean(i_star, x) +
               self.get_mean(i_star, i)) / (self.KL_plus(self.get_mean(i, x), 0.5))
          if self.KL_plus(self.get_mean(i, x), 0.5) != 0 else 0) 
          for x in np.arange(self.K) if x != i])
    ...
    def algo(self):
        ...
        while self.t < self.T:
            ...
            self.L_r = self.L_n
            self.L_n = np.arange(0)
        return list(np.around(self.regrets,3)), np.argmin(self.I_t), self.t
